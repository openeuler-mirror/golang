...
func ProfileGuided(fn *ir.Func, p *pgoir.Profile) {
	...
	edit = func(n ir.Node) ir.Node {
		...
		if *db >= 2 {
			fmt.Printf("%v: %s devirtualize considering call %v\n", ir.Line(call), mode, call)
		}

		if call.GoDefer {
			if *db >= 2 {
				fmt.Printf("%v: can't %s devirtualize go/defer call %v\n", ir.Line(call), mode, call)
			}
			return n
		}
...
func maybeDevirtualizeInterfaceCall(p *pgoir.Profile, fn *ir.Func, call *ir.CallExpr) (ir.Node, *ir.Func, int64) {
	_, _, _, _, _, dv, h = base.CFGOSwitch()
	if *dv < 1 {
		return nil, nil, 0
	}
	...
	if !(*h).MatchPosWithInfo(call.Pos(), "devirt", nil) {
		return nil, nil, 0
	}
...
func maybeDevirtualizeFunctionCall(p *pgoir.Profile, fn *ir.Func, call *ir.CallExpr) (ir.Node, *ir.Func, int64) {
	db, _, _, _, _, dv, h = base.CFGOSwitch()
	if *dv < 2 {
		return nil, nil, 0
	}
	...
	if base.Flag.LowerM > 1 || logopt.Enabled() {
		defer func() {
			if reason != "" {
				...
				if base.ENABLE_CFGO {
					mode = "CFGO"
					log = ": should not CFGO devirtualize function"
				} else {
					mode = "PGO"
					log = ": should not PGO devirtualize function"
				}
				if base.Flag.LowerM > 1 {
					fmt.Printf("%v: should not %s devirtualize %v: %s\n", ir.Line(fn), mode, ir.FuncName(fn), reason)
				}
				if logopt.Enabled() {
					logopt.LogOpt(fn.Pos(), log, "pgoir-devirtualize", ir.FuncName(fn), reason)
				}
			}
		}()
	}
...
func rewriteInterfaceCall(call *ir.CallExpr, curfn, callee *ir.Func, concretetyp *types.Type) ir.Node {
	...
	if base.Flag.LowerM != 0 {
		fmt.Printf("%v: %s devirtualizing interface call %v to %v\n", ir.Line(call), mode, call.Fun, callee)
	}
	...
	if *db >= 3 {
		fmt.Printf("%s devirtualizing interface call to %+v. After: %+v\n", mode, concretetyp, res)
	}
...
func rewriteFunctionCall(call *ir.CallExpr, curfn, callee *ir.Func) ir.Node {
	...
	if base.Flag.LowerM != 0 {
		fmt.Printf("%v: %s devirtualizing function call %v to %v\n", ir.Line(call), mode, call.Fun, callee)
	}
	...
	if *db >= 3 {
		fmt.Printf("%s devirtualizing function call to %+v. After: %+v\n", mode, ir.FuncName(callee), res)
	}
