...
func bytesStringBytesOpt(f *Func) {
	...
	if !isEnableBytesStringBytesOpt() {
		return
	}
	...
	for _, b := range f.Blocks {
		for _, v := range b.Values {
			...
			isNil := tmpBuf.Op == OpConstNil ||
				(tmpBuf.Op == OpARM64MOVDconst && auxIntToInt64(tmpBuf.AuxInt) == 0)
			if !isNil {
				continue
			}
	...
	for _, m := range matches {
		...
		taddr := b.NewValue1A(v.Pos, OpARM64MOVDaddr, cfgTypes.BytePtr, reflectdata.TypeLinksym(types.Types[types.TUINT8]), sb)
		...
		mc := b.NewValue0A(v.Pos, OpARM64CALLstatic, aux2.LateExpansionResultType(), aux2)
