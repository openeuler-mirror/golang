...
func (gcToolchain) gc(b *Builder, a *Action, archive string, importcfg, embedcfg []byte, symabis string, asmhdr bool, pgoProfile string, cfgoProfile string, gofiles []string) (ofile string, output []byte, err error) {
	p := a.Package
	sh := b.Shell(a)
	objdir := a.Objdir
	if archive != "" {
	...
	if cfgoProfile != "" {
		defaultGcFlags = append(defaultGcFlags, "-cfgoprofile="+cfgoProfile)
	}
	if symabis != "" {
