...
var stdout = flag.Bool("stdout", false, "write to stdout instead of sizeclasses_expand_span_size.go")

func main() {
	flag.Parse()

	var b bytes.Buffer
	fmt.Fprintln(&b, "// Code generated by mksizeclasses_expand.go; DO NOT EDIT.")
	fmt.Fprintln(&b, "//go:generate go run mksizeclasses_expand.go")
	fmt.Fprintln(&b)
	if !*stdout {
		fmt.Fprintln(&b, "//go:build arm64 && goexperiment.pagenum")
	}
	...
	if *stdout {
		...
		err = os.WriteFile("sizeclasses_expand_span_size.go", out, 0666)
	}
	if err != nil {
		log.Fatal(err)
	}
...
const (
	...
	pageNumberMultiplierForMinSize = 8
	pageNumberMaxMultiplier        = 8
)
...
func makeClasses() []class {
	...
	for i := range classes {
		...
		// 52 indicates 8x pages for object under 8KB (min span 8KB*8=64KB)
		// 60 indicates 4x pages for object under 16KB (min span 8KB*4=32KB)
 		if i == 6 {
			c.npages *= pageNumberMaxMultiplier / 2
		} else if i < 52 {
			c.npages *= pageNumberMaxMultiplier
		} else if i < 60 {
			c.npages *= pageNumberMaxMultiplier / 2
		}
...
func printClasses(w io.Writer, classes []class) {
	...
	fmt.Fprintf(w, "_PageNumberMultForMinSize = %d\n", pageNumberMultiplierForMinSize)
	fmt.Fprintf(w, "_PageNumberMaxMult = %d\n", pageNumberMaxMultiplier)
	fmt.Fprintln(w, ")")
